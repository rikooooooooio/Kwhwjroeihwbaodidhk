...
QuestNeta = function()
  ...
  return {
    [1] = Mon,           -- nome do monstro
    [2] = LevelQuest,    -- nível/id da quest (era Qdata)
    [3] = NameQuest,     -- nome da quest (era Qname)
    [4] = NameMon,     -- posição dos monstros (era PosM)
    [5] = CFrameQuest,       -- nome do monstro para exibição
    [6] = CFrameMon    -- posição do NPC (era PosQ)
  }
end

local Fluent = loadstring(game:HttpGet("https://github.com/dawid-scripts/Fluent/releases/latest/download/main.lua"))()
local Window = Fluent:CreateWindow({Title = "Frost Hub | Blox Fruits",SubTitle = "rikobby",TabWidth = 180,Size = UDim2.fromOffset(660, 440),Acrylic = false,Theme = "darker",MinimizeKey = Enum.KeyCode.End})
...
ToggleButton.Image = "rbxassetid://120173730959357" -- sua imagem
...
-- ========== CONFIGURAÇÕES ==========
local Sec = 0.2                     -- intervalo entre ações (segundos)
local MaxDistanceToNPC = 10         -- distância para pegar a missão
local MaxDistanceToMonster = 150    -- se monstro estiver mais longe, teleporta para área
local AttackDelay = 0.2             -- delay entre ataques

-- ========== FUNÇÕES AUXILIARES (garantia) ==========
...
local function getCurrentQuestTitle()
    ...
    if not questContainer or not questContainer.Visible then return nil end
...
-- Abandona a missão atual
local function abandonQuest()
    game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("AbandonQuest")
    task.wait(0.5)
end

-- Pega a missão (usando os dados do QuestNeta)
local function startQuest()
    local data = QuestNeta()
    game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("StartQuest", data[3], data[2])
end
...
task.spawn(function()
    while true do
        task.wait(Sec)
        if not _G.Level then continue end  -- se desligado, pula
        ...
        pcall(function()
            ...
            local monsterName = questData[1]      -- ex: "Isle Champion"
            local questNameRequired = questData[5] -- nome da missão esperado (NameMon)
            local questCFrame = questData[6]       -- Posição do NPC
            local farmCFrame = questData[4]        -- Área dos monstros

            local currentTitle = getCurrentQuestTitle()
            local hasQuest = (currentTitle ~= nil)

            -- Se tem missão mas o título não corresponde à esperada, abandona
            if hasQuest and currentTitle and not string.find(currentTitle, questNameRequired) then
                abandonQuest()
                hasQuest = false
                task.wait(0.5)
            end
            ...
            if not hasQuest then
                -- SEM MISSÃO: vai até o NPC e pega a missão
                if (root.Position - questCFrame.Position).Magnitude > MaxDistanceToNPC then
                ...
                -- COM MISSÃO ATIVA: farma os monstros
                local enemies = workspace:FindFirstChild("Enemies")
                local target = nil
                if enemies then
                    for _, enemy in ipairs(enemies:GetChildren()) do
                        if enemy.Name == monsterName and isEnemyAlive(enemy) then
                            target = enemy
                            break
                        end
                    end
                ...
                if target then
                    -- Se o monstro está longe, teleporta para a área de farm
                    if (root.Position - target.HumanoidRootPart.Position).Magnitude > MaxDistanceToMonster then
                        teleportTo(farmCFrame)
                    else
                        -- Ataca o monstro (uma vez por iteração, com delay)
                        pcall(function()
                    ...
                    -- Nenhum monstro encontrado: teleporta para a área de farm
                    teleportTo(farmCFrame)
                end
            end
        end)
